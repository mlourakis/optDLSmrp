% wrapper around Nakano-based MRP solver; implements sequential rotations by pi directly on matrix M
...
function [R t minrepr] = optDLS_mrp_seqrot(p3, p2, chk_singular)
  ...
  %R0 = cat(3, rrotx(pi), rroty(pi), rrotz(pi));
  R0 = cat(3,...
          [ 1 0 0; 0 1 0; 0 0 1],...   % eye(3)
          [ 1 0 0; 0 -1 0; 0 0 -1],... % rrotx(pi)
          [-1 0 0; 0  1 0; 0 0 -1],... % rroty(pi)
          [-1 0 0; 0 -1 0; 0 0  1]);   % rrotz(pi)
  ...
  for j=1:4
    R0j=R0(:,:,j);
    G=[R0j zeros(3,6); zeros(3,3) R0j zeros(3,3); zeros(3,6) R0j];
    M0=G*M*G'; % apply pre-rotation

    % solve
    [b c d]=solver_optDLS_mrp(M0, chk_singular);
    nsol=size(b,2);
    if(nsol==0)
      continue;
    end
    ...
    %if minerr~=Inf, break; end % stop at the first reliable solution
  end
  ...
  if(minerr==Inf)
    ...
    %warning('optDLS_mrp_seqrot: no solutions found, singular rotation?');
    return;
  end

  %fprintf('optDLS_mrp_seqrot: minimum reprojection error %f for MRP vector [%f %f %f]\n', minerr, minmrp(1), minmrp(2), minmrp(3));
